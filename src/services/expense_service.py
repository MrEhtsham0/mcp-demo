...
class ExpenseService:
        ...
        async with AsyncSession(async_engine) as session:
            try:
                ...
                async with session.begin():
                    ...
                    # No need for explicit commit() when using session.begin()
                
                # Refresh the expense to get the generated ID
                ...
                # Transaction will be automatically rolled back due to session.begin()
    ...
    async def get_expenses_by_date_range(
        ...
        async with AsyncSession(async_engine) as session:
            try:
                # Read operations don't need explicit transactions, but we can use them for consistency
                async with session.begin():
    ...
    async def get_all_expenses() -> List[Dict[str, Any]]:
        ...
        async with AsyncSession(async_engine) as session:
            try:
                # Read operations with transaction for consistency
                async with session.begin():
